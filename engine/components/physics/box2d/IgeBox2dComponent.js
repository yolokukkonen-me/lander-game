...
var IgeBox2dComponent = IgeEventingClass.extend({
	...
	start: function () {
		if (!this._active) {
			...
			// CRITICAL FIX: Bind context to preserve 'this' in setInterval
			var self = this._entity;
			this._intervalTimer = setInterval(function() {
				self.box2d._behaviour.call(self);
			}, 1000 / 60);
		}
			}
		}
	...
	_behaviour: function (ctx) {
		// Check if box2d component still exists
		if (!this.box2d) {
			return;
		}
	...
	// Optimized: 6 velocity, 2 position iterations for better performance
	if (self._mode === 0) {
		self._world.Step(ige._tickDelta / 1000, 6, 2);
	} else {
