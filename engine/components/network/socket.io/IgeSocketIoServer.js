var IgeSocketIoServer = {
	...
	start: function (data, callback) {
		...
		// Start socket.io with Socket.IO 4.x API
		this.log('Starting socket.io listener on port ' + this._port);
		var Server = this._socketio.Server;
		this._io = new Server(this._port, {
			cors: {
				origin: "*",
				methods: ["GET", "POST"]
			}
			...
			if (clientId) {
				if (typeof(clientId) === 'object') {
					...
					while (arrCount--) {
						...
						if (clientSocket) {
							clientSocket.emit('message', [commandIndex, data]);
						} else {
				...
				this._io.emit('message', [commandIndex, data]);
			}
		} else {
